...
def waitRand():
    waitms = randint(5000, 15000) / 1000
    sleep(waitms)
...
def main():
    ...
    while curpage <= totpages or totpages == 0:
        try:
            url = "https://www.consob.it/web/area-pubblica/oscuramenti?p_p_id=com_liferay_asset_publisher_web_portlet_AssetPublisherPortlet_INSTANCE_m9PTOY4SM1GU&_com_liferay_asset_publisher_web_portlet_AssetPublisherPortlet_INSTANCE_m9PTOY4SM1GU_cur={}".format(
                curpage
            )
            if curpage > 1:
                waitRand()
            ...
            for div in soup.find_all("div", attrs={"class": "divContent"}):
                for comunicato in div.find_all("h4"):
                    ...
                    for url in extractor.find_urls(content):
                        ext = extract(url)
                        if ext.domain == "consob" and ext.suffix == "it":
                            continue
                        if ext.domain == "europa" and ext.suffix == "eu":
                            continue
                        url = (
                            (ext.subdomain + "." + ext.domain + "." + ext.suffix)
                            if ext.subdomain != ""
                            else (ext.domain + "." + ext.suffix)
                        )
                        urls.append(url)
            curpage = curpage + 1
        except Exception:
